...
class BeamSearch(object):
    def __init__(self, model, max_len: int, eos_idx: int, beam_size: int, config):
        self.model = model
    ...
    def _expand_visual(self, visual: utils.TensorOrSequence, cur_beam_size: int, selected_beam: torch.Tensor):
        if isinstance(visual, torch.Tensor):
            selected_beam = selected_beam.to(self.device_b)
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs, att = self.iter(t, visual, outputs, return_probs, **kwargs)
        ...
        if return_probs:
            ...
            # return outputs, log_probs, att #zjk
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        word_logprob, vis_mat = self.model.step(t, self.selected_words, visual, None, mode='feedback', **kwargs)
